...
import { Mail, Phone, MapPin, Globe, Linkedin, Calendar, Award, Briefcase, GraduationCap, ShoppingBag, FileText, Users, Star, Tag, TrendingUp } from 'lucide-react';

// Cấu hình cho template Retail
...
// Template Retail class
class RetailTemplate extends CVTemplateBase {
  constructor() {
    super(retailConfig);
  }
          ...
          <div className="max-w-5xl mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-center">
              <div className="text-center md:text-left">
                ...
                <p className="text-orange-100 mt-1 text-lg">{personalInfo.title}</p>
              </div>
              {personalInfo.avatar && (
          ...
          <div className="max-w-5xl mx-auto">
            <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm justify-center md:justify-start">
              ...
              {personalInfo.website && (
                <div className="flex items-center">
                  <Globe size={14} className="text-orange-600 mr-2" />
                  <span className="text-slate-700">{personalInfo.website}</span>
                </div>
              )}
              {personalInfo.linkedin && (
                <div className="flex items-center">
                  <Linkedin size={14} className="text-orange-600 mr-2" />
                  <a href={personalInfo.linkedin} className="text-slate-700 hover:text-orange-600 transition-colors">
                    LinkedIn
                  </a>
                </div>
              )}
        ...
        <div className="p-8 max-w-5xl mx-auto">
          ...
          {cvData.getSummary() && (
            <section className="mb-8">
              <h2 className="text-2xl font-bold text-orange-600 border-b-2 border-orange-200 pb-2 mb-4 flex items-center">
                <FileText className="inline mr-2" size={22} /> Professional Summary
              </h2>
              <div className="bg-orange-50 rounded-lg p-5 shadow-sm border border-orange-100">
          ...
          {experiences.length > 0 && (
            <section className="mb-8">
              <h2 className="text-2xl font-bold text-orange-600 border-b-2 border-orange-200 pb-2 mb-6 flex items-center">
                <Briefcase className="inline mr-2" size={22} /> Retail Experience
              </h2>
              <div className="space-y-6">
                {experiences.map((exp) => (
                  <div key={exp.id} className="border-l-4 border-orange-200 pl-4 hover:border-orange-500 transition-colors">
                    <div className="flex flex-col md:flex-row justify-between items-start mb-2">
                      ...
                      <div className="mt-1 md:mt-0 bg-orange-100 text-orange-700 text-sm px-3 py-1 rounded-full flex items-center">
          ...
          {skills.length > 0 && (
            <section className="mb-8">
              <h2 className="text-2xl font-bold text-orange-600 border-b-2 border-orange-200 pb-2 mb-6 flex items-center">
                <Tag className="inline mr-2" size={22} /> Retail Skills
              </h2>
                ...
                {skills.filter(skill => skill.category === 'Technical').length > 0 && (
                  <div className="bg-white rounded-lg p-5 shadow-sm border border-orange-100">
                    <h3 className="text-lg font-semibold text-orange-600 mb-4 flex items-center">
                      <ShoppingBag className="inline mr-2" size={18} /> Retail Operations
                    </h3>
                  ...
                  <div className="bg-white rounded-lg p-5 shadow-sm border border-orange-100">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {skills
                        ...
                        .map(skill => (
                          <span key={skill.id} className="bg-orange-100 text-orange-600 px-3 py-2 rounded-full text-sm">
                            {skill.name}
                          </span>
                        ))}
                    </div>
          ...
          {education.length > 0 && (
            <section className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {education.map((edu) => (
                  <div key={edu.id} className="bg-white rounded-lg p-5 shadow-sm border border-orange-100">
                    <h3 className="text-lg font-semibold text-orange-600">{edu.degree}</h3>
                ...
                <div className="space-y-3">
                  {certifications.map((cert, index) => (
                    <div key={index} className="bg-white rounded-lg p-4 shadow-sm border border-orange-100">
                      <h4 className="font-medium text-orange-600">{cert.name}</h4>
              ...
              <section className="mb-8">
                ...
                <div className="space-y-3">
                  {languages.map((lang, index) => (
                    <div key={index} className="flex justify-between items-center bg-white rounded-lg p-3 shadow-sm border border-orange-100">
                      <span className="text-slate-700 font-medium">{lang.name}</span>
                      <span className="text-orange-600 text-sm bg-orange-100 px-3 py-1 rounded-full">{lang.level}</span>
                    </div>
                  ))}
                </div>
              </section>
          ...
          {projects.length > 0 && (
            <section>
              <h2 className="text-2xl font-bold text-orange-600 border-b-2 border-orange-200 pb-2 mb-6 flex items-center">
                <TrendingUp className="inline mr-2" size={22} /> Sales Campaigns & Projects
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {projects.map((project) => (
                  <div key={project.id} className="bg-white rounded-lg p-5 shadow-sm border border-orange-100 hover:shadow-md transition-shadow">
                    <h3 className="text-lg font-semibold text-orange-600 mb-2">{project.name}</h3>
...
const retailTemplateInstance = new RetailTemplate();

const TemplateRetailComponent: React.FC<CVTemplateProps> = (props) => {
  return retailTemplateInstance.render(props);
};

export default withTemplateBase(TemplateRetailComponent, retailTemplateInstance);
export { retailTemplateInstance };